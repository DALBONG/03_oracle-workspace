...
--2. 고객들이 산 상품을 확인할 수 있도록 고객명, 상품이름, 개수, 구입날짜를 VIEW로 출력(뷰 이름 VW_TBL, 구입날짜 출력은 0000년, 00월, 00일)							
		
-- 현아님			
-- 2023년에 판매되지 않는 상품을 상품코드순으로 정렬하여 출력한다.							
-- 구매번호가 짝수인 거래 내역을 대상으로, 각 상품의 판매 수량이 전체 판매량에서 차지하는 비율을 계산하여 출력한다.							
...
--2. TBL_CUSTOM 테이블에서 나이(AGE)가 30세 이상인 고객 중. 이메일 주소가 ''korea.com' 도메인을 포함하는 고객의 
    -- CUSTOM_ID, NAME, EMAIL, AGE와 해당 고객들이 최근 구매한 날짜(BUY_DATE)를 출력하고, 
    -- 만약 구매 기록이 없으면 '구매기록없음'으로 표시하시오. (단 결과는 나이 내림차순, 이름 오름차순으로 정렬할 것)		
...
-- 우석님
--1. 고객이 회원가입을 하면, 가입일로부터 30일 이내에 사용할 수 있는 웰컴쿠폰이 1장 자동으로 발급되고, 해당 쿠폰은 구매 시 자동으로 적용된다고 가정한다. 이 조건에 따라, 가입 후 30일 이내에 상품을 구매하여 쿠폰을 사용한 고객들의 ID를 중복 없이 조회하시오.							
...
-- 민준님                         		
--1. 이메일을 다음을 쓰는 고객의 이름, 구매한 상품명, 각 상품별 구매 개수를 조회하세요. (이름빠른 순서로 정렬)							
...
-- 다현님
--1. 24년도 이전에 가입한 고객들 중 총구매수량이 가장 높은 고객의 이름, 이메일, 총구매가격, 총구매수량을 출력하세요.							
